...
export default class BinarySerializer extends EventEmitter {
	static serializeMessage(message) {
		...
		buf.append(BinarySerializer._serializeValue(message.getLength()));
		buf.append(BinarySerializer._serializeValue(message.type));

		for (let i = 0; i < message.getLength(); i++) {
	...
	onValue(value) {
		...
		if (count === -1) {
			...
			if (currentMessage.getLength() === count) {
